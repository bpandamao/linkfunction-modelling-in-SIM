...
#X = data.matrix(fromJSON('X.json'))
#Y = data.matrix(fromJSON('Y.json'))
#XE = data.matrix(fromJSON('XE.json'))
#YE = data.matrix(fromJSON('YE.json'))
...
#   KC=matrix(nr=S,nc=n)
  ...
  for (isample in 1:length(keep)){
    dblist.est = dbspline(we.pred, GG_knots[isample,1:(KK[isample]-d+1)], d)
    sr.est = bsplinePsd:::densityMixture(GG[isample,1:KK[isample]], dblist.est)
    sr.est = pmax(sr.est, ee)
    YE.est[,isample]= mu_f(AA[isample]*sr.est,baseb)
  }
...
w=function(i){
  ...
  outa=gibbs_bspline_est_linkfunction(X,Y,S=200000,burnin=100000,thin=10,YE=YE,XE=XE,logstate=FALSE)
  outb=gibbs_bernstein_polynomial_est_linkfunction(X,Y,S=200000,burnin=100000,thin=10,YE=YE,XE=XE,logstate=FALSE)
  out=list(a=outa,b=outb)
  
  return(out)
}


results = parLapply(cl, 1:30, w)
write(toJSON(results),'./results/all_results.json')
